...
def weighted_determine_interfacial_level(candidates, threshold=5):
    """
...
# Setup server
server = Server()
server.set_endpoint("opc.tcp://0.0.0.0:4840/reactor/level/") 

# Setup address space
name = "InterfacialLevel"
uri = "http://reactor.local/levelsensor"
idx = server.register_namespace(uri)

objects = server.get_objects_node()
level_obj = objects.add_object(idx, "LevelSensor")

# Expose only LevelValue
level_var = level_obj.add_variable(idx, "LevelValue", 0.0)
level_var.set_writable(False)   # PLC only reads


# Start server
server.start()
print("OPC UA Server started at opc.tcp://192.168.1.30:4840")

#try:
#    while True:
        # Replace with your real-time level calculation
#        level_value = weighted_determine_interfacial_level()
#        level_var.set_value(level_value)
#        time.sleep(1)
...
# Let the user select the overall ROI on the first frame
...
# Physical dimensions for calibration (you must measure these)
...
try:
    while True:
        ...
        new_interfacial_level = weighted_determine_interfacial_level(candidates, threshold=5)
        ...
        if new_interfacial_level is not None:
            ...
            # 1) Publish **only** the physical height to OPC UA
            level_var.set_value(h_cm)

            # 2) (Optional) print to console for debugging
            print(f"[OPC UA] Published interfacial height: {h_cm:.2f} cm")
